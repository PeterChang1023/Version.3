...
function Compute_Corrections_Init(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  if (do_water_correction)
    ...
    sum_tracers_p    = Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, grid_tracers_all, grid_ps_p)
  end
...
function Compute_Corrections!(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  if (do_water_correction) 
    ...
    for i in collect(1:128)
        for j in collect(1:64)
            for k in collect(1:20)
    ...
    sum_tracers_n    = Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, grid_tracers_all, grid_ps_n)

    tracers_correction = (sum_tracers_p - (sum_tracers_n-abs(Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, total, grid_ps_n)))) # sum(total) is negative
